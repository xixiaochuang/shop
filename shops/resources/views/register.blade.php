        ...
        <div class="registerCon">
            <ul>
                <li class="accAndPwd">
                    ...
                    <dl>
                        <input type="text" id="userMobile" placeholder="请输入验证码" style="width: 80%;">
                ...
                <li>@csrf<a id="btnNext" href="javascript:;" class="orangeBtn loginBtn">下一步</a></li>
            </ul>
        </div>
...
<div class="footer clearfix" style="display:none;">
    <ul>
        <li class="f_home"><a href="/v44/index.do" style=" pointer-events: "><i></i>云购</a></li>
...
<script>
    ...
    $('#btnNext').click(function(){
    	...
    	var obj={};
    	obj._token=$(this).prev().val();
    	obj.admin_tel=$('#userMobile').val()
        obj.admin_pwd=$('.pwd').val()
        obj.password=$('.conpwd').val();
    	$.post(
    	    "{{url('userpage/login/add')}}"
            ,obj
            ,function(res){
               if(res==1){
                   alert('添加成功');
    ...
    $('.sendcode').click(function(){
        ...
        var value=$('.sendcode').text('4');
        var set=setInterval(times,1000);
        function times() {
           var nums=parseInt($('.sendcode').text());
           var ss=nums-1;
            $('.sendcode').text(ss)
            if(nums==1){
                clearInterval(set)
