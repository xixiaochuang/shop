        ...
        <div class="all-list-wrapper">
            ...
            <div class="menu-list-wrapper" id="divSortList">
                ...
                <ul id="sortListUl" class="list">
                    <li sortid='0' class='current cate'><span class='items'p_id="0">全部商品</span></li>
                    @foreach($cate as $v)
                       {{--// {{dd($k)}}--}}
                    <li sortid='100' reletype="1" linkaddr=''class='cate '><span class='items'p_id="{{$v->cate_id}}">{{$v->cate_name}}</span></li>
                        @endforeach
                </ul>
            </div>
            ...
            <div class="good-list-wrapper">
                <div class="good-menu thin-bor-bottom">
                    <ul class="good-menu-list" id="ulOrderBy">
                        <li orderflag="10" class="shop current" type="1"><a href="javascript:;">综合</a></li>
                        <li orderflag="20" class="shop" type="2" fileds="goods_num"><a href="javascript:;">人气</a></li>
                        <li orderflag="50"  class="shop" type="2" fileds="create_time"><a href="javascript:;">最新</a></li>
                        <li orderflag="30"  class="shop" type="3" fileds="self_price"><a href="javascript:;">价值</a><span class="i-wrap">'↑'</span></li>
                        <!--价值(由高到低30,由低到高31)-->
                    </ul>
                </div>
                                ...
                                <li id="23468">    
                                    ...
                                    <div class="gList_r">        
                                        ...
                                        <div class="gRate">            
                                            ...
                                            <a codeid="12785750" class="shang" canbuy="646" goods_id="{{$v->goods_id}}"><s></s></a>
                                        </div>
                                    </div>
                                </li>
                                    @endforeach
...
<script>
    ...
    $(function(){
            ...
            $('.cate').click(function(){
                var cate_id=$(this).children('span').attr('p_id');
                var _token=$(this).parents('ul').prev().val();
                {{--$.post(--}}
                {{--"{{url('allshops/shopinfo')}}"--}}
            ...
            function shopinfo(){
                var obj={};
                var cate_id=$('.cate.current').children('span').attr('p_id');
                var _token=$('.cate').parents('ul').prev().val();
                var shop=$('.shop.current');
                var type=shop.attr('type')
                if(type==1){
                    var fileda=null;
                    var ord=null;
                }else if(type==2){
                    var fileda=shop.attr('fileds');
                    var ord=null;
                }else {
                    var fileda=shop.attr('fileds');
                    var od=shop.children('span').text();
                    if(od=='↑'){
                        var ord='asc';
                        shop.children('span').text('↓');
                    }else{
                        var ord='desc';
                        shop.children('span').text('↑')
                ...
                $.post(
                    "{{url('allshops/shopinfo')}}"
                    ,{_token:_token,cate_id:cate_id,fileds:fileda,ord:ord}
                    ,function(res){
                        $('.goodList').html(res);
                    }
                )
           ...
           // $('.shang').click(function(){
                ...
                $.ajax({
                    ...
                    if(res.icon==2){
                        layer.msg(res.font,{icon:res.icon,time:1000},function(){
